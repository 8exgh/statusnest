...
function describe(name: string, status: 'online' | 'offline' | 'unknown', checkedAt: Date | undefined, uptime24h: number | null, uptime30d: number | null, now: Date): string {
  const state = status === 'unknown' ? 'has not been checked yet' : `is ${statusWord(status)} as of our last check ${relativeTime(checkedAt, now)}`;
  const windows = uptime24h === null ? '' : ` 24-hour uptime ${formatUptime(uptime24h)}, 30-day uptime ${formatUptime(uptime30d)}.`;
  return `${name} ${state}.${windows} StatusNest loads ${name} in a real Chromium browser every 5–20 minutes.`;
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const description = describe(site.name, site.status, site.lastCheckedAt, detail?.uptime.last24h.uptime ?? null, detail?.uptime.last30d.uptime ?? null, now);
...
export default async function SiteStatusPage({ params }: Params) {
  ...
  if (!data) notFound();
  const { site, primary, uptime, checks24h, daily90, incidents30d, pageUptime24h } = data;
  const url = absoluteUrl(`/status/${site.slug}`);
  ...
  const faq: FaqItem[] = [
    ...
    },
    {
      ...
      answer: `Roughly every 15 minutes. Each visit is scheduled at random between 5 and 20 minutes after the previous one, so the checks never fall into a predictable pattern. We check ${pageCount} ${site.name} page${pageCount === 1 ? '' : 's'}: ${site.pages.map((p) => p.name).join(', ')}.`,
    },
    {
      question: 'What does "unavailable" mean?',
      answer:
        'The page did not load in our browser: the server returned an error (HTTP 4xx or 5xx), the page timed out after 30 seconds, the address could not be resolved or connected to, or the site showed a bot challenge instead of the real page. A bot challenge is flagged separately because it usually means the site is up but is blocking automated visitors.',
    },
    {
  ...
  const jsonLd = [
    ...
    },
    {
      ...
      description: describe(site.name, site.status, site.lastCheckedAt, uptime.last24h.uptime, uptime.last30d.uptime, now),
      url,
  ...
  return (
    <>
      ...
      <Section title={`How StatusNest checks ${site.name}`}>
        <Methodology siteName={site.name} pages={site.pages} siteSlug={site.slug} />
      </Section>
