...
export default function Methodology({ siteName, pages, siteSlug }: MethodologyProps) {
  return (
    <div className="space-y-3 text-sm leading-relaxed text-gray-700">
      <p>
        Roughly every 15 minutes — each visit is scheduled at random between 5 and 20 minutes after the last one, so checks never
        fall into a predictable pattern — a <strong>real Chromium browser</strong> running on a virtual display (Xvfb) in our data
        centre opens {pages.length === 1 ? 'this page' : `each of the ${pages.length} ${siteName} pages we track`}
        {pages.length > 1 && (
          <>
            :{' '}
            {pages.map((p, i) => (
      ...
      <p>
        ...
        load within 30 seconds, the address could not be resolved or connected to, or the site showed a bot challenge instead of
        the page — that last case is flagged separately, because it usually means the site is up but blocking automated visitors.
      </p>
      <p>
